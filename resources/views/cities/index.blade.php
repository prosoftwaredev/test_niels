<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-center items-center">
            ...
            <a href="{{ route('cities.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                {{ __('Add City') }}
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('cities.index') }}" method="GET" role="search">
                            ...
                            @foreach ($cities as $city)
                                <tr>
                                    <td class="px-3 py-2 whitespace-no-wrap">
                                        <a href="{{ route('cities.edit', ['city' => $city->id]) }}" class="underline text-gray-600 hover:text-gray-900">
                                        {{ $city->name }}
                                    </td>
                                    <td class="px-3 py-2 whitespace-no-wrap">{{ $city->state->name }}</td>
