<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-center items-center">
            ...
            <a href="{{ route('countries.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                {{ __('Add Country') }}
            </a>
        </div>
    </x-slot>
                            ...
                            @foreach ($countries as $country)
                                <tr>
                                    <td class="px-3 py-2 whitespace-no-wrap">
                                        <a href="{{ route('countries.edit', ['country' => $country->id]) }}" class="underline text-gray-600 hover:text-gray-900">
                                        {{ $country->name }}
                                    </td>
                                    <td class="px-3 py-2 whitespace-no-wrap">{{ $country->country_code }}</td>
                                    <td class="px-3 py-2 whitespace-no-wrap flex flex-row">
                                        <form method="POST" action="{{ route('countries.destroy', ['country' => $country->id]) }}">
                                            @csrf
                                            @method("DELETE")
